...
CREATE POLICY IF NOT EXISTS "Users can only access their own tasks" ON user_tasks
  FOR ALL USING (user_id = current_user::text);

CREATE POLICY IF NOT EXISTS "Users can only access their own study plans" ON user_study_plans
  FOR ALL USING (user_id = current_user::text);

CREATE POLICY IF NOT EXISTS "Users can only access their own exam dates" ON user_exam_dates
  FOR ALL USING (user_id = current_user::text);
...
CREATE TRIGGER IF NOT EXISTS update_user_study_plans_updated_at 
  BEFORE UPDATE ON user_study_plans 
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER IF NOT EXISTS update_user_exam_dates_updated_at 
