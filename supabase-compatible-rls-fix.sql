-- Supabase互換のRLSポリシー修正スクリプト
-- forcerowsecurityカラムの問題を修正
...
-- テーブルのRLS状態（forcerowsecurityを除外）
...
DROP POLICY IF EXISTS "authenticated_user_tasks_access" ON user_tasks;
DROP POLICY IF EXISTS "authenticated_user_study_plans_access" ON user_study_plans;
DROP POLICY IF EXISTS "authenticated_user_exam_dates_access" ON user_exam_dates;
...
-- user_tasksテーブル用ポリシー
CREATE POLICY "supabase_user_tasks_policy" ON user_tasks
  FOR ALL 
  TO authenticated
...
CREATE POLICY "supabase_user_study_plans_policy" ON user_study_plans
  FOR ALL 
  TO authenticated
  USING (user_id = auth.uid()::text)
  WITH CHECK (user_id = auth.uid()::text);
...
CREATE POLICY "supabase_user_exam_dates_policy" ON user_exam_dates
  FOR ALL 
  TO authenticated
  USING (user_id = auth.uid()::text)
  WITH CHECK (user_id = auth.uid()::text);
...
-- 成功メッセージ
SELECT 'Supabase-compatible RLS policies have been successfully created!' as final_status;
...
-- 現在の認証ユーザーIDを確認
SELECT 'Current authenticated user:' as test_step, auth.uid() as current_user_id;

-- テーブルアクセステスト（エラーが出なければ成功）
SELECT 'Access test completed' as test_result;